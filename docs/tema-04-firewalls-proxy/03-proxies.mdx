...
sequenceDiagram
    participant Cliente
    participant Proxy as Proxy Directo
    participant Internet
    participant Servidor as Servidor Web
    
    Cliente->>Proxy: Solicitud HTTP
    Proxy->>Internet: Reenvía solicitud
    Internet->>Servidor: Solicitud
    Servidor->>Internet: Respuesta
    Internet->>Proxy: Respuesta
    Proxy->>Cliente: Respuesta HTTP
```
...
```mermaid
graph LR
    Cliente[Cliente Internet] --> Proxy[Proxy Inverso]
    Proxy --> Balanceador[Balanceador de Carga]
    Balanceador --> Pool[Pool de Servidores]
    Pool --> S1[Servidor Web 1]
    Pool --> S2[Servidor Web 2]
    Pool --> S3[Servidor Web 3]
