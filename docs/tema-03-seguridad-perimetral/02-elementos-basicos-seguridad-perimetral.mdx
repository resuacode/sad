...
```
                  ...
                  (Externo)
                     ...
                     [DMZ]
            +----------+----------+
       Servidor Web  Servidor Mail  Proxy
                       |
                  [Firewall 2]
                  (Interno)
                       |
...
[Firewall Externo]
   |
   +-- [DMZ Pública] - Servidores web de cara al público
   |
[Firewall Intermedio]
   |
   +-- [DMZ Corporativa] - Portales de empleados, VPN
   |
[Firewall Interno]
   |
[Red Interna]
...
```
         ...
         +------------+------------+
         |            |            |
    DMZ (eth1)   LAN (eth2)   Gestión (eth3)
```
...
**Arquitectura típica**:
```
Internet ←→ [Router Frontera] ←→ [Firewall] ←→ [Switch Core] ←→ Red Interna
                  │                    │                │
              Enrutamiento         Seguridad      Distribución
                  NAT              Filtrado        VLANs
              QoS Básico         IDS/IPS          QoS
```
