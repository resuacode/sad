...
Usuario Remoto
      |
      | 1. Solicitud de conexión (credenciales)
      ↓
[Pasarela de Acceso]
      |
      | 2. Validación contra servidor AAA
      ↓
[Servidor RADIUS/LDAP/TACACS+]
      |
      | 3. Respuesta: Aceptar/Rechazar
      ↓
[Pasarela de Acceso]
      |
      | 4. Establecer/Denegar conexión
      ↓
Red Corporativa
```
...
```
Usuario Remoto (RDP Client)
         |
         | HTTPS (443) - Túnel SSL/TLS
         ↓
[RD Gateway Server]
         |
         | Validación con AD
...
[Active Directory]
         ...
         | RDP interno (3389)
         ↓
[Terminal Servers / Escritorios Virtuales]
```
...
Administrador
    |
    | 1. SSH al Bastion (puerto 22)
    | ssh -i ~/.ssh/id_rsa admin@bastion.empresa.com
    ↓
[Bastion Host]
    |
    | 2. Desde el Bastion, SSH al servidor destino
    | ssh usuario@servidor-interno.local
    ↓
[Servidor Interno]
```
...
```
Usuario                NAS/Gateway             Servidor RADIUS           Backend
   |                        |                         |                      |
   |--1. Login Request----->|                         |                      |
   |   (usuario/password)   |                         |                      |
   |                        |--2. Access-Request----->|                      |
...
sequenceDiagram
    ...
    Router-->>Admin: 6. Login exitoso
    Admin->>Router: 7. show running
    Router->>TAC: 8. AUTHOR/REQUEST (comando: show run)
    TAC-->>Router: 9. AUTHOR/ACCEPT
    Router-->>Admin: 8. Config display
    Router->>TAC: 10. ACCT/START
    Admin->>Router: 9. configure term
    Router->>TAC: 10. AUTHOR/REQUEST (comando: config t)
    TAC-->>Router: 11. AUTHOR/REJECT
    Router-->>Admin: 10. Access denied
```


```
Administrador          Router/Switch         Servidor TACACS+
     |                       |                        |
     |--1. Login: admin----->|                        |
     |                       |--2. AUTHEN/START------>|
     |                       |   (username: admin)    |
...
```
Usuario → VPN Gateway → RADIUS Server → Duo Cloud
  |            |              |              |
  +--(1)------>|              |              |
  |  Login     |              |              |
  |            +--(2)-------->|              |
