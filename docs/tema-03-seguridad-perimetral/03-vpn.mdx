...
```
┌─────────────────────────────────────────────────┐
│           Paquete Original (Payload)            │
│  ┌──────────┬─────────────────────────────┐    │
│  │ IP Header│      Datos Aplicación        │    │
│  └──────────┴─────────────────────────────┘    │
│                                                  │
│              ↓ Encapsulación VPN ↓              │
│                                                  │
│  ┌─────────────────────────────────────────┐   │
│  │ Nuevo IP  │ VPN      │ IP    │  Datos   │   │
│  │ Header    │ Header   │ Orig. │  Orig.   │   │
│  │ (Público) │ (Cifrado)│       │          │   │
│  └─────────────────────────────────────────┘   │
│                                                  │
│         Paquete Transmitido por Internet        │
└─────────────────────────────────────────────────┘
```
...
```
Usuario ──[HTTP sin cifrar]──> Router WiFi Hotel ──> Internet
                                    ↑
                              Atacante con
                           Wireshark captura
...
**Con VPN**:
```
Usuario ──[Túnel cifrado AES-256]──> Router WiFi Hotel ──> VPN Gateway ──> Internet
                                           ↑
                                    Atacante solo ve
                                    datos cifrados
                                   (indescifrables)
...
```
Empleado remoto (Casa)
      ↓ VPN
[Servidor VPN] ←→ [Red Interna]
...
**Vectores de ataque**:
```
Dispositivo Remoto Comprometido
         │
         ├─ Malware local
         ├─ Keylogger captura credenciales
         ├─ Ransomware se propaga por VPN
         └─ Backdoor para acceso persistente
                │
                ↓ VPN
          [Red Corporativa]
         (infectada desde remoto)
```
...
└─────────────┘  │
                 │
┌─────────────┐  │    Internet    ┌──────────────┐      ┌──────────────┐
│ Usuario 2   │──┼───────────────→│ VPN Gateway  │──────│ Red Interna  │
│ (Café)      │  │                 │ (Servidor)   │      │ Corporativa  │
└─────────────┘  │                 └──────────────┘      └──────────────┘
                 │
┌─────────────┐  │
│ Usuario 3   │──┘
...
```
┌────────────────────────┐                    ┌────────────────────────┐
│   Oficina Central      │                    │   Sucursal Remota      │
│                        │                    │                        │
│  ┌──────────────────┐  │                    │  ┌──────────────────┐  │
│  │ Servidores       │  │                    │  │ PCs Locales      │  │
│  │ Bases de Datos   │  │                    │  │ Impresoras       │  │
│  └────────┬─────────┘  │                    │  └────────┬─────────┘  │
│           │            │                    │           │            │
│    ┌──────┴──────┐     │                    │    ┌──────┴──────┐     │
│    │   Switch    │     │                    │    │   Switch    │     │
...
│    │ (Router/FW) │     │  Túnel IPSec       │    │ (Router/FW) │     │
│    └─────────────┘     │    Cifrado         │    └─────────────┘     │
│                        │                    │                        │
│  Red: 10.0.0.0/24      │                    │  Red: 10.1.0.0/24      │
└────────────────────────┘                    └────────────────────────┘
```
...
│                      │                    │          │           │
│  ┌────────────────┐  │                    │  ┌───────┴────────┐  │
│  │ API Compras    │◄─┼────────────────────┼─►│ API Pedidos    │  │
│  │ (Compartido)   │  │   VPN Extranet     │  │ (Compartido)   │  │
│  └────────────────┘  │                    │  └────────────────┘  │
│                      │                    │                      │
│  Acceso Limitado     │                    │  Acceso Limitado     │
└──────────────────────┘                    └──────────────────────┘
```
...
```
Usuario ──[Navegador Web]──> https://vpn.empresa.com
                                    ↓
                          Portal Web de Acceso
                                    ↓
                      ┌──────────────┴──────────────┐
                      │                             │
            Acceso a Aplicaciones Web      Escritorio Remoto
            (Intranet, SharePoint)         (RDP, VNC)
```
...
*Modo Túnel (Cliente)*:
```
Usuario ──[Cliente SSL VPN]──> VPN Gateway
                                    ↓
                            Red Completa
                        (como VPN tradicional)
```
...
```
┌──────────┬──────────┬───────────┐
│ IP Header│ ESP Header│ Datos     │
│ Original │ (Cifrado) │ (Cifrados)│
└──────────┴──────────┴───────────┘
```
...
```
┌─────────┬──────────┬──────────┬──────────┬─────────┐
│ Nuevo IP│ESP Header│IP Original│  Datos   │ESP      │
│ Header  │          │+ Payload  │Originales│Trailer  │
│         │          │(Cifrados) │          │         │
└─────────┴──────────┴──────────┴──────────┴─────────┘
```
...
```
Objetivo: Establecer canal seguro para negociar
┌─────────┐                              ┌─────────┐
│Cliente  │ ────1. Propuesta────────────>│Servidor │
│         │ <───2. Aceptación────────────│         │
...
│         │ <───6. Verificación──────────│         │
└─────────┘                              └─────────┘
   Resultado: Túnel ISAKMP seguro (SA)
```

*Fase 2 (IKE Phase 2 / Quick Mode)*:
```
Objetivo: Negociar parámetros del túnel datos
┌─────────┐                              ┌─────────┐
│Cliente  │ ────1. Propuesta IPSec───────>│Servidor │
│         │ <───2. Aceptación IPSec──────│         │
│         │ ────3. Confirmación──────────>│         │
└─────────┘                              └─────────┘
   Resultado: Túnel IPSec operativo (SA)
```
...
└──────────┘    │                              │    └──────────┘
              ...
              └─┬─┘                          └─┬─┘
                │                              │
              ┌─┴───────────────────────────────┴─┐
              │    Red MPLS del Proveedor         │
              │  ┌───┐   ┌───┐   ┌───┐   ┌───┐   │
              │  │PE │───│P  │───│P  │───│PE │   │
              │  └───┘   └───┘   └───┘   └───┘   │
              │                                    │
              │  Etiquetas MPLS segregan tráfico  │
              └────────────────────────────────────┘

CE = Customer Edge (router cliente)
PE = Provider Edge (router proveedor)
P  = Provider Core (core del proveedor)
```
...
```
        ...
        ┌───────────────┼───────────────┐
        │               │               │
    MPLS (Primario)  IPSec (Backup)  SSL VPN
        │               │             (Usuarios)
        │               │               │
    Sucursal A      Sucursal B      Remotos
  (Permanente)    (Temporal)      (Móviles)
```
...
**Arquitectura**:
```
Cliente ──[TCP 1723]──> Servidor PPTP
            │                │
         Control          Control
            │                │
Cliente ──[GRE]────────> Servidor PPTP
       (Datos cifrados   (Túnel)
        con MPPE)
```
...
│              Encapsulación L2TP                        │
│  ┌──────┬──────┬────────────────────────────┐         │
│  │ IP   │ L2TP │  IP Orig + Datos           │         │
│  └──────┴──────┴────────────────────────────┘         │
│                      ↓                                 │
│              Cifrado IPSec                             │
│  ┌──────┬──────┬────────────────────────────────┐     │
│  │ IP   │ ESP  │ L2TP + IP Orig + Datos         │     │
│  │ Ext  │      │      (Cifrado AES)             │     │
│  └──────┴──────┴────────────────────────────────┘     │
│                                                        │
│         Paquete transmitido por Internet               │
...
┌──────┬──────┬────────────────────┬──────────┐
│ IP   │ ESP  │     Payload        │   ESP    │
│Header│Header│   (Cifrado AES)    │  Trailer │
└──────┴──────┴────────────────────┴──────────┘
         │                              │
         └─── Cifrado ────────────────┘
              Autenticación e Integridad
...
**IKEv1 (RFC 2409)**:
```
Fase 1 - ISAKMP SA (Main Mode o Aggressive Mode):
┌─────────┐                                    ┌─────────┐
│Iniciador│                                    │Responder│
└────┬────┘                                    └────┬────┘
     │                                              │
     │──1. SA Proposal (algoritmos)───────────────>│
     │<─2. SA Accept (selecciona algoritmos)───────│
     │──3. Key Exchange (DH public key)───────────>│
     │<─4. Key Exchange (DH public key)────────────│
     │──5. Identity + Auth (cifrado con DH key)──>│
     │<─6. Identity + Auth (cifrado con DH key)────│
     │                                              │
     └──────── Túnel ISAKMP establecido ───────────┘
              (Protege Fase 2)

Fase 2 - IPSec SA (Quick Mode):
     │──1. IPSec Proposal (transform sets)────────>│
     │<─2. IPSec Accept────────────────────────────│
     │──3. Confirm─────────────────────────────────>│
     │                                              │
     └───── Túnel IPSec operativo para datos ──────┘
```

**IKEv2 (RFC 7296)** - Versión moderna:
```
Simplificación: Solo 4 mensajes (vs 6-9 en IKEv1)

┌─────────┐                                    ┌─────────┐
│Iniciador│                                    │Responder│
└────┬────┘                                    └────┬────┘
     │                                              │
     │──IKE_SA_INIT (SA, KE, Nonce)──────────────>│
     │<─IKE_SA_INIT (SA, KE, Nonce)───────────────│
     │                                              │
     │──IKE_AUTH (IDi, AUTH, SAi2, TSi, TSr)─────>│
     │<─IKE_AUTH (IDr, AUTH, SAr2, TSi, TSr)──────│
     │                                              │
     └──── Túnel completo en 2 round-trips ────────┘
```
...
│                  Cliente OpenVPN                       │
│  ┌──────────────────────────────────────────────┐     │
│  │  Aplicación (SSH, HTTP, etc.)                │     │
│  └───────────────────┬──────────────────────────┘     │
│                      │                                 │
│  ┌───────────────────▼──────────────────────────┐     │
│  │  Interfaz Virtual (tun0 / 10.8.0.2)          │     │
│  └───────────────────┬──────────────────────────┘     │
│                      │                                 │
│  ┌───────────────────▼──────────────────────────┐     │
│  │  OpenVPN Process                              │     │
│  │  - Cifra con AES-256-GCM                      │     │
│  │  - Encapsula en SSL/TLS                       │     │
│  │  - Envuelve en UDP/TCP                        │     │
│  └───────────────────┬──────────────────────────┘     │
└────────────────────┬─┴────────────────────────────────┘
                     │
...
│                      │                               │
│  ┌───────────────────▼──────────────────────────┐   │
│  │  Interfaz Virtual (tun0 / 10.8.0.1)          │   │
│  └───────────────────┬──────────────────────────┘   │
│                      │                               │
│                      ▼                               │
│               [Red Interna LAN]                      │
...
```
┌─────────────────┐                          ┌──────────────────────┐
│ Empleado Remoto │                          │  Servidor OpenVPN    │
│                 │                          │  (Ubuntu 22.04)      │
│  - Laptop       │      Internet            │                      │
│  - Cliente VPN  │◄─────────────────────────►│  IP: 203.0.113.10   │
│  - Certificado  │   Túnel Cifrado          │  Puerto: 1194/UDP   │
│                 │   AES-256-GCM            │                      │
└─────────────────┘                          └──────────┬───────────┘
                                                        │
                                                        │
...
```
┌───────────────────────────────────────┐              ┌───────────────────────────────────────┐
│       Oficina Central (Vigo)        │              │     Sucursal Remota (Ourense)       │
│                                       │              │                                       │
│  Red LAN: 192.168.1.0/24              │              │  Red LAN: 192.168.2.0/24              │
│                                       │              │                                       │
│  ┌─────────────────────────────────┐  │              │  ┌─────────────────────────────────┐  │
│  │  Servidores, PCs, Impresoras    │  │              │  │  PCs, Impresoras                │  │
│  └──────────────┬──────────────────┘  │              │  └──────────────┬──────────────────┘  │
│                 │                     │              │                 │                     │
│          ┌──────┴──────┐              │              │          ┌──────┴──────┐              │
│          │   Switch    │              │              │          │   Switch    │              │
...
│                 │                     │              │                 │                     │
│       ┌─────────┴─────────┐           │              │       ┌─────────┴─────────┐           │
│       │ Gateway WireGuard │           │              │       │ Gateway WireGuard │           │
│       │ wg0: 10.99.0.1/24 │           │              │       │ wg0: 10.99.0.2/24 │           │
│       │ eth0: 192.168.1.1 │           │              │       │ eth0: 192.168.2.1 │           │
│       └─────────┬─────────┘           │              │       └─────────┬─────────┘           │
│                 │                     │              │                 │                     │
│         IP Pública: 203.0.113.10      │              │         IP Pública: 198.51.100.20     │
└─────────────────┼─────────────────────┘              └─────────────────┼─────────────────────┘
                  │                                                      │
                  │                    Internet                          │
                  │            ═══════════════════════════               │
...
```
┌─────────────────────────────────┐         ┌─────────────────────────────────┐
│    Sede Central (10.0.0.0/24)   │         │    Sucursal (10.1.0.0/24)       │
│                                 │         │                                 │
│    ┌────────────────────┐       │         │    ┌────────────────────┐       │
│    │ Gateway strongSwan │       │         │    │ Gateway strongSwan │       │
│    │ eth0: 203.0.113.10 │       │         │    │ eth0: 198.51.100.20│       │
│    │ eth1: 10.0.0.1     │       │         │    │ eth1: 10.1.0.1     │       │
│    └────────┬───────────┘       │         │    └────────┬───────────┘       │
│             │                   │         │             │                   │
└─────────────┼───────────────────┘         └─────────────┼───────────────────┘
              │                                           │
